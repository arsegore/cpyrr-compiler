...
build/lex.yy.c: src/grammaire build/y.tab.c
	@echo "$(RED)\n→ Génération du lex$(NC)"
	mkdir -p build
	flex -o build/lex.yy.c src/grammaire/grammaire.l

build/lex.yy.o: build/lex.yy.c
	$(CC) $(CFLAGS) -c build/lex.yy.c -o build/lex.yy.o

build/y.tab.c build/y.tab.h: src/grammaire
	@echo "$(RED)\n→ Génération du yacc$(NC)"
	mkdir -p build inc
	byacc -dv -o build/y.tab.c src/grammaire/grammaire.y
